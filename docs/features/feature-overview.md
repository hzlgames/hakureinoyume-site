...
- `/tools/ZJU_tools`：登录用户保存或删除自己的 ZJU 学号、密码和可选 Pintia Cookie。凭据只加密保存在服务端，不回传明文。
...
- `/tools/ZJU_tools/courses.zju/materials`：课程资料，支持课程选择、资料搜索、可见项全选、选中文件下载、任务日志、取消运行任务和下载产物。
- 资料下载任务写入当前用户自己的任务记录和工作目录，可在页面查看日志、取消未完成任务、下载已完成文件。

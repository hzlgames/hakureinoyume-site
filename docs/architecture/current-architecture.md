...
- Prisma 7 + PostgreSQL 存储用户、会话、账号、验证 token、管理员审计日志、网易云账号/网页歌单和 ZJU 工具数据。
- Nodemailer 负责认证邮件发送。
- 网易云播放器通过 `/api/music/*` 代理自托管 NeteaseCloudMusicApi Enhanced 或兼容服务，用户级网易云 cookie 加密入库。
...
- Tools UI：`/tools` 提供工具入口；`/tools/ZJU_tools` 管理用户级 ZJU 凭据并按服务分组展示工具索引；学在浙大（`courses.zju`：待办、成绩、资料下载、自动刷课、测验答案）、智云课堂（`classroom.zju`：录播回放与转录）、图书馆（`lib.zju`：借阅续借）、WebPlus（`webplus.zju`：通知存档）各有独立工具页面。
- API Routes：`src/app/api/**/route.ts` 提供 Better Auth、后台用户管理、背景图、天气、节日日历、网易云音乐代理和 ZJU 工具接口。
...
- `src/app/api/music/[...music]/route.ts`：代理网易云搜索、歌单、专辑、播放地址、歌词、二维码登录和用户网页歌单。
- `src/app/api/zju/**`：登录用户的 ZJU 账号、学在浙大课程/待办/成绩/资料/活动、智云课堂课程与录播、图书馆借阅与续借、互动测验和工具任务接口。账号接口只要求 Better Auth session；多数具体工具接口要求当前用户已有通过验证的 ZJU 账号，并按 `userId` 读取凭据或隔离任务。
...
- `GET /api/zju/quiz/courses`：读取测验答案工具可选课程列表，要求已验证 ZJU 账号，并使用当前用户的 `COURSES` 客户端。
...
- `GET|POST /api/zju/jobs`：列出当前用户最近 ZJU 工具任务，或创建任务（课程资料下载 `courses.zju/materialDown`、自动刷课 `courses.zju/autoplay`、测验答案 `courses.zju/quiz`、课堂转录 `classroom.zju/transcript`、WebPlus 存档 `webplus.zju/archive`），要求已验证 ZJU 账号。API 输出会过滤内部工作目录和文件绝对路径。
